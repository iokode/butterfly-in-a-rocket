/**
 * Lucide glyph paths, shared by the Astro and Svelte icon components so the two can never carry
 * different artwork for the same name.
 *
 * The design system loads Lucide from a CDN because its UI kits run standalone in a browser off a
 * single HTML file. This site is statically built, so the paths are inlined instead — same glyphs,
 * same ISC licence, no runtime dependency, and no request that has to resolve before an icon
 * appears.
 */

// Path data from lucide-static 0.462.0 (ISC).
export const icons = {
    ...
    /* Callout tones. The design system maps one glyph to each tone and does not vary them. */
